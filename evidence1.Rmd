...
print("Como conclusion, puedo decir que la tendencia es que cuan más grande es la secuencia,")
print("el contendio de GC aumenta, lo cual suena razonable. Sin embargo como toda regla,")
print("tiene su excepcion como lo arroja la variante XBB.1.9.1  y la variante BA.2.75.2. ")
print("Aunado a esto, me gustaria recalcar que las variantes VOI tienden a tener una ")
    print("longitud mayor a las que tienen otra clasificacion asignada por la SIG. ")
...
nucleotide_df <- data.frame(Virus = rep(sequence_names, each = 4),
                            Nucleotide = rep(c("A", "C", "G", "T"), times = length(sequence_names)),
                            Frequency = c(a, c, g, t))

# Graficar
ggplot(nucleotide_df, aes(x = Virus, y = Frequency, fill = Nucleotide)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(title = "Frecuencia de Nucleótidos por Variante de Virus",
       x = "Variante de Virus",
       y = "Frecuencia",
       fill = "Nucleótido") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_manual(values = c("#FF5733", "#3498DB", "#58D68D", "#F4D03F")) +  
  geom_text(aes(label = sprintf("%.2f", Frequency)), position = position_stack(vjust = 0.5), size = 3) 

print("Por último, me gustaria agregar que las variables estudiadas ALFA, ZETA ")

print("son en las que debemos enfocarnos mas, estas concuerdan en estar en la clasificacion de VOC,")
print(" por lo que su alto porcentaje en ")
print("nucletodidos A puede tener relacion")
```
